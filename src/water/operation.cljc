(ns water.operation
  "OperationActor -- one water-utility operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Water Advisor) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Water Safety Governor (:govern) and the
  ...
  One graph run = one water-utility operation (intake -> advise ->
  ...
  decision to a human operator (the utility officer). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:report/publish`/`:alert/suppress` ALWAYS reach this node when the
  governor is clean -- see `water.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [water.wateradvisor :as wateradvisor]
...
(defn build
  ...
  `water.store/Store`).
  opts:
    :advisor      -- a `water.wateradvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (wateradvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Water Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (wateradvisor/-advise advisor store request)]
            {:proposal p :audit [(wateradvisor/trace request p)]})))

      ;; Water Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
